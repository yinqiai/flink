...
object KafkaConnector {
  def main(args: Array[String]): Unit = {
   val env = StreamExecutionEnvironment.getExecutionEnvironment
    ...
    // kafka 配置
    val ZOOKEEPER_HOST = "hadoop000:2181"
    val KAFKA_BROKERS = "hadoop000:9092"
   ...
   val result=streamData.flatMap(_.toLowerCase().split(" "))
     .filter(_.nonEmpty)
     .map((_,1))
     .keyBy(0).sum(1)
     //.flatMap(_.toLowerCase().split(" ").filter(_.nonEmpty).map((_,1))).keyBy(0).sum(1)
   ...
   val result=streamData//.flatMap(a=>{print(a.getName);List(a)})
    // .flatMap(_.getName.map((_,1))).keyBy(0).sum(1)
     .flatMap(_.getName.map((_,1))).keyBy(0).sum(1)
      result.print("jisuan")
